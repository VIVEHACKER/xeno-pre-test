...
// ============================================================
// Auth — access token은 메모리 변수에만 보관 (localStorage 금지).
// refresh token은 서버가 설정한 HttpOnly 쿠키(cpa_refresh).
// ============================================================

let accessToken = null; // 모듈 레벨 메모리 전용
let currentUser = null; // {id, email, role}
let appBooted = false; // 앱 부트가 한 번이라도 됐는지
...
// {error:{code,message,details}} 봉투를 파싱해 사람이 읽을 메시지로.
...
async function rawAuthFetch(path, body) {
...
// 모든 백엔드 호출의 단일 진입점.
// - Authorization: Bearer <access> 자동 첨부 (있을 때)
// - credentials:"include" 로 refresh 쿠키 동반
// - 401 → /auth/refresh 1회 시도 → 성공 시 원 요청 1회 재시도
//   refresh 실패 시 토큰 비우고 로그인 화면.
async function apiFetch(path, opts = {}, _retried = false) {
	...
	try {
		...
		// 네트워크 단절 — 호출자가 status로 구분할 수 있도록 표식.
	...
	if (res.status === 401 && !_retried) {
		...
		if (refreshed) {
			return apiFetch(path, opts, true); // 원 요청 1회 재시도
		}
		// refresh 실패 → 세션 종료
		clearAccessToken();
		showLoginGate();
...
// /auth/refresh 1회 시도. 성공 시 access 저장하고 true.
async function tryRefresh() {
	try {
		const data = await rawAuthFetch("/auth/refresh");
...
function defaultSubjectState() {
	...
	for (const id of SUBJECT_ORDER) {
		state[id] = {
			...
			tags:
				id === "accounting" || id === "tax"
					? ["time_pressure", "objective_entry_delay"]
					: [],
		};
	}
	return state;
...
function updateSubjectTableMeta() {
	...
	if (meta)
		meta.textContent = `${active}개 과목 활성 / ${SUBJECT_ORDER.length}`;
}
...
function buildPayload() {
	...
	// user_id 제거 — 서버가 access token에서 신원을 도출함.
	return {
		target_exam: "CPA_1",
...
async function postDiagnose(payload) {
	// apiFetch 경유: Authorization 헤더 + 401 refresh 재시도 + 에러 봉투 파싱.
	return apiJson("/diagnose", "POST", payload);
}
	...
	$("#priorityList").innerHTML = concepts
		.map((concept, i) => {
			const width = Math.max(
				50,
				Math.round(100 - i * (50 / Math.max(1, concepts.length - 1))),
			);
			return `
...
function renderTasks(rx) {
	...
	$("#taskList").innerHTML = rx.daily_tasks
		.map((task) => {
			const subj = SUBJECT_LABEL[task.subject] ?? escapeHtml(task.subject);
...
function classifyLearner(payload) {
	...
	const avgAccuracy =
		states.reduce((sum, s) => sum + s.accuracy, 0) / states.length;
	...
	if (avgAccuracy < 0.5) {
		return {
			...
			defer: "고난도 모의고사",
			reason:
				"평균 정답률이 50% 아래라 기출 회독보다 개념-풀이 연결을 먼저 복구해야 합니다.",
		};
	}
	if (maxOverrun >= 0.4) {
		return {
			...
			defer: "풀이 오래 붙잡기",
			reason:
				"정답률보다 시간초과율이 더 큰 병목입니다. 넘김 기준과 풀이 순서를 먼저 고정합니다.",
		};
	}
	if (stage === "objective_entry") {
		return {
			...
			defer: "기본서 정독 회귀",
			reason:
				"기본 개념을 문제 선택지로 바꾸는 단계입니다. 설명보다 판별 기준을 훈련합니다.",
		};
	}
	if (days <= 45 || stage === "final") {
		return {
			...
			defer: "새로운 범위 확장",
			reason:
				"남은 기간이 짧아 새 범위보다 반복 실수와 시간 손실을 줄이는 편이 유리합니다.",
		};
	}
	return {
		level: "회독 안정화",
		...
		defer: "무근거 양치기",
		reason:
			"현재는 많은 문제보다 왜 틀렸는지 남는 회독 구조를 만드는 단계입니다.",
	};
}
...
function scoreProblemForToday(problem, weakSubject, concepts, payload) {
	const weakState = payload.subject_states.find(
		(state) => state.subject === weakSubject,
	);
	...
	if (
		overrun >= 0.35 &&
		problem.question_analysis?.question_type === "calculation"
	)
		score += 7;
	if (
		payload.current_stage === "objective_entry" &&
		problem.solution_paths?.length >= 3
	)
		score += 6;
	return score;
}
...
function buildDailyPrescription(payload, rx = null) {
	...
	const concepts = uniqueItems([
		...(rx?.concepts_to_review || []),
		profile.focus,
	]).slice(0, 4);
	const problems = selectTodayProblems(payload, concepts);
	const hours = payload.available_hours_per_day;
	const problemMinutes = Math.max(
		45,
		Math.min(120, Math.round(hours * 60 * 0.32)),
	);
	const reviewMinutes = Math.max(
		15,
		Math.min(60, Math.round(hours * 60 * 0.14)),
	);
	...
	const otherUnits = problemSolutionMaps
		...
		.map(
			(problem) =>
				`${SUBJECT_LABEL[displaySubjectKey(problem)] ?? problem.subject} ${unitLabel(problem.unit)}`,
		);
	const deferItems = uniqueItems([
		profile.defer,
		weakState.time_overrun_rate >= 0.35
			? "시간을 넘긴 문제를 끝까지 붙잡기"
			: "채점 없는 문제 양치기",
		...otherUnits,
	...
	return {
		...
		verification:
			rx?.weekly_goal?.verification_metric ||
			"오늘 문제별 선택지 제거 근거와 오답 원인 태그를 남깁니다.",
		problems,
		concepts,
		deferItems,
...
function renderCoachFromState(payload, rx = null) {
	...
	$("#coachGuidelines").innerHTML = guidelines
		.map((item) => `<li>${escapeHtml(item)}</li>`)
		.join("");
...
function renderEvidence(rx) {
	...
	$("#evidenceSummary").textContent =
		`${rx.evidence_refs.length}개 근거. 클릭해서 원본 조회.`;
...
async function loadEvidenceDetail(refType, refId) {
	...
	try {
		const data = await apiFetch(
			`/evidence/${refType}/${encodeURIComponent(refId)}`,
		);
...
function filteredTutorials() {
	...
	return subjectTutorials.filter(
		(tutorial) => phase === "all" || phaseOptionValue(tutorial) === phase,
	);
}
...
function renderTutorialSubjectOptions() {
	...
	if (!tutorials.length) {
		$("#tutorialSubjectSelect").innerHTML =
			`<option value="">과목 없음</option>`;
		return;
	}
	if (
		!tutorials.some((tutorial) => tutorial.tutorial_id === selectedTutorialId)
	) {
...
function filteredProblemMaps() {
	...
	return problemSolutionMaps.filter(
		(item) => subject === "all" || displaySubjectKey(item) === subject,
	);
}
...
function renderProblemFilters() {
	...
	$("#problemSubjectFilter").innerHTML = [
		...
		...ordered.map(
			(value) =>
				`<option value="${escapeAttr(value)}">${escapeHtml(SUBJECT_LABEL[value] ?? value)}</option>`,
		),
	].join("");
...
function renderProblemOptions() {
	...
	if (!items.length) {
		$("#problemSolutionSelect").innerHTML =
...
function updateAttemptClock() {
	$("#problemAttemptElapsed").textContent = formatElapsed(
		currentAttemptSeconds(),
	);
}
	...
	setAttemptStatus({
		selected:
			selectedAttemptChoice === null
				? "미선택"
				: `${selectedAttemptChoice + 1}번`,
		result: latestAttemptDiagnosis
			? latestAttemptDiagnosis.correct
				? "정답"
				: "오답"
			: "대기",
...
function renderAttemptDiagnosis(diagnosis, persistenceLabel, explanation) {
	...
	updateChoiceSelection();
	$("#problemCorrectAnswer").textContent =
		`정답 ${diagnosis.correct_choice + 1}번`;
...
async function submitProblemAttemptDiagnosis() {
	...
	try {
		...
		showToast(`채점 실패: ${err.message}`, "error");
	}
}
...
function renderQuestionAnalysis(item) {
	...
	if (!analysis) {
		...
		$("#problemIntent").innerHTML =
			`<p class="muted-note">출제 의도 분석이 아직 생성되지 않았습니다.</p>`;
...
$("#runAttemptDiagnosis").addEventListener(
	"click",
	submitProblemAttemptDiagnosis,
);
...
function renderDataManifest(manifest) {
	...
	$("#trainableAfterReviewCount").textContent =
		stats.trainable_after_review_jobs ?? 0;
	$("#blockedLearningJobCount").textContent =
		stats.learning_status?.blocked ?? 0;
	...
	$("#solutionConceptLinkCount").textContent =
		stats.solution_concept_links ?? 0;
	...
	$("#problemSolutionPathCount").textContent =
		stats.problem_solution_paths ?? 0;
	$("#problemChoiceEliminationCount").textContent =
		stats.problem_choice_eliminations ?? 0;
...
async function healthCheck() {
	try {
		...
		setStatusBar(
			`엔진 준비됨 · 규칙 ${data.decision_rules} · 문제 ${data.problems}`,
			"ok",
		);
		return true;
	} catch (err) {
		setStatusBar(
			`엔진 연결 실패 — cpa-serve 실행 필요 (${err.message})`,
			"error",
		);
		return false;
	}
...
function renderAuthMode() {
	...
	$("#authTogglePrompt").textContent = isSignup
		? "이미 계정이 있으신가요?"
		: "계정이 없으신가요?";
	$("#authToggleBtn").textContent = isSignup ? "로그인" : "회원가입";
...
function showAuthError(message) {
	const el = $("#authError");
	if (!el) return;
	el.textContent = message; // textContent — 서버 문자열을 HTML로 해석하지 않음 (XSS 방지)
	el.hidden = false;
}
...
async function handleAuthSubmit(event) {
	...
	try {
		const data = await rawAuthFetch(path, { email, password });
		setAccessToken(data.access_token);
...
async function handleLogout() {
	try {
		await rawAuthFetch("/auth/logout");
	} catch {
		/* 서버 로그아웃 실패해도 클라이언트 상태는 비운다 */
	}
	clearAccessToken();
	setTopbarUser(null);
	showLoginGate();
...
// 인증 성공 후: 앱 셸 표시 + 1회 부트 + 저장된 상태 rehydrate.
async function enterApp() {
	...
	if (ok) await hydrateFromPrescription();
}

// 저장된 처방을 GET /prescription 으로 복원. 신규 사용자(404)는 fresh diagnose.
async function hydrateFromPrescription() {
	try {
		const body = await apiFetch("/prescription");
		const rx = body.prescription || body; // 응답 형태 방어
		latestPrescription = rx;
		render(rx);
		setStatusBar("저장된 처방 복원됨", "ok");
	} catch (err) {
		if (err.status === 404) {
			// 신규 사용자 — 현재 입력 기준으로 새 진단.
			triggerDiagnose();
			return;
		}
		if (err.status === 401) {
			// apiFetch가 이미 로그인 게이트 처리.
			return;
		}
		// 그 외 오류는 fresh diagnose로 폴백 (앱은 계속 동작).
		triggerDiagnose();
	}
}

// 인증 무관하게 한 번만 실행하는 정적/로컬 초기화.
function bootApp() {
	renderSubjectTable();
...
// 부트: 복귀 사용자는 유효한 refresh 쿠키를 가질 수 있으므로 먼저 refresh 시도.
async function boot() {
	initAuthControls();
	const refreshed = await tryRefresh();
	if (refreshed) {
		// /auth/me 로 사용자 정보 보강 (refresh 응답에 user가 없을 수 있음).
		if (!currentUser) {
			try {
...
async function runTermSearch(q) {
	...
	if (!q) {
		...
		resultsBox.innerHTML =
			'<p class="terms-empty">왼쪽 검색창에 용어를 입력하세요.</p>';
		return;
	}
	meta.textContent = `"${q}" 검색 중…`;
	try {
		const data = await apiFetch(
			`/terms/search?q=${encodeURIComponent(q)}&limit=30`,
		);
...
async function loadTermDetail(termId) {
	...
	try {
		const data = await apiFetch(`/terms/${encodeURIComponent(termId)}`);
