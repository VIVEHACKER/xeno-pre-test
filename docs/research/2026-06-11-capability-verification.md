...
codex 백엔드, 164문항(math_lossy 제외):

| 과목 | 유효 정답률 | 보정 정답률 | 진짜 오답 | 미해결 |
|---|---|---|---|---|
| **상법(corporate_law)** | **100% (38/38)** | 95% (38/40) | 0 | 2 (codex 600초 초과) |
| business | 100% (23/23) | 92% (23/25) | 0 | 2 (사용량 한도) |
| economics | 100% (9/9) | 90% (9/10) | 0 | 1 (사용량 한도) |
| accounting | 94% (47/50) | 94% | 3 | 0 |
| tax | 87.2% (34/39) | 87.2% | 5 | 0 |
| **전체** | **95.0% (151/159)** | **92.1% (151/164)** | 8 | 5 |

세 수치의 의미:
- **원시 86.6%(142/164)**: 첫 실행의 codex 240초 타임아웃에 깎인 하한. 오답 22개 중
  17개가 `chose=-1`(답 추출 실패)였고, 재실행(타임아웃 600초)하니 9개가 정답으로 회복 —
  타임아웃은 지식 실패가 아니라 인프라 절단이었음(예: 상법 Q26은 재실행 시 `ANSWER: 3` 정답).
- **보정 92.1%**: 미해결 5개를 오답으로 보수 처리.
- **유효 95.0%**: 미해결 5개(600초 초과 2 + codex 사용량 한도 3) 제외.

**핵심**: 메모리에 "상법 실기출 검증 부재 — 합성 15문항 93%만 존재"로 남아 있던 갭이
**실제 상법 기출 40문항 100%(유효)/95%(보정)로 해소**. "합성 95~97%는 천장"이라던 우려도
실기출 전 과목 87~100%로 재현됨. 합격 기준(평균 60%·과목 40%) 전 과목 크게 상회.
...
1. ~~실제 기출 PDF 미수집~~ **해소(2026-06-13)** — 금융감독원 공식 2026 1차 기출
   직접 수집·파싱·실측(§1.1). 단 2024~2026 3개년·1차만. 2차·CTA·구년도 미수집.
2. ~~상법 실기출 검증 부재~~ **해소** — 실제 상법 40문항 유효 100%/보정 95%(§1.1).
3. **경제 수식 문항 측정 불가** — 경제 32문항 중 22개가 PDF 수식 폰트 PUA 유실로 제외.
   현재 텍스트 파이프라인으론 풀이 불가 — 페이지 렌더링+비전 모델 경로가 있어야 측정 가능.
4. **codex 지연·사용량 한도** — 문항당 80~600초(일부 600초 초과). 실시간 부적합, 배치 전용.
   사용량 한도로 3문항 미측정. 운영 ai-explain은 anthropic 권장(이미 반영).
5. **로컬 단독 세법 과락권** — 라우팅으로 해소하지만 클라우드 의존(비용/오프라인 제약).
6. **ai-explain 로컬 지연** — qwen3.5:27b 기준 문항당 2~6분 실측. 미설정 시 503 차단.
7. **UI 미통합** — 프로토타입 UI는 새 처방 필드·신규 엔드포인트를 아직 렌더링 안 함.
8. **decision rules 35개는 수동 작성(machine_draft)** — 합격수기 자동 추출(M4)은 docs-only.
9. **추천 가중치는 휴리스틱** — 실사용 로그 기반 보정은 데이터 누적 후.
10. **수집 문항 RAG/학습 미투입** — `train_after_rights_review` 정책. 현재 eval 전용.
